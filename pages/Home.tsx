    ...
    <div className="relative">
      ...
      <section className="relative min-h-screen flex items-center justify-center pt-20 overflow-hidden bg-gradient-to-b from-black via-[#030308] to-black">
        ...
        <div className="relative z-10 max-w-4xl px-6 text-center">
          ...
          <p className="text-xl md:text-2xl text-slate-400 font-light italic mb-10 max-w-3xl mx-9">
            Análise de dados, Dashboards, automações e soluções digitais.
          </p>
      ...
      <section className="pt-20 pb-44 bg-gradient-to-b from-[#0f0f14] via-[#13131a] to-[#0f0f14] relative overflow-hidden border-t border-white/5">
        ...
        {/* <div className="w-full border-y border-white/5 bg-black/50 backdrop-blur-sm py-6 mt-10 relative z-20">
              ...
              <div className="animate-scroll flex gap-12 whitespace-nowrap min-w-full">
                {[...TOOLS, ...TOOLS, ...TOOLS].map((tool, i) => (
                  <span key={i} className="text-lg font-semibold text-slate-500 flex items-center gap-3 hover:text-brand-400 transition-colors">
                      <span className="w-1.5 h-3 rounded-full bg-brand-500/70"></span> {tool}
                  </span>
                ))}
              </div>
            </div>
...
const FundamentalCard: React.FC<{ icon: React.ReactNode; title: string; text: string }> = ({ icon, title, text }) => (
  <div className="group relative bg-[#18181d]/50 backdrop-blur-sm border border-white/10 rounded-2xl p-8 min-h-[240px] flex flex-col hover:bg-[#1a1a20]/70 hover:border-brand-500/30 transition-all duration-300">
    <div className="flex items-start gap-4 mb-6">
      <div className="p-2  bg-[#0f0f14]/80 rounded-xl border border-white/10 group-hover:border-brand-500/20 group-hover:bg-[#0a0a0f] transition-all">
        {icon}
      </div>
